...
- (void)baiduMap
{
    ...
    BOOL ret = [_mapManager start:@"EiY4zZPTavG2GgRyOVBR2a9W"  generalDelegate:nil];
    ...
    [tool requestCompletion:^(NSDictionary *result, NSError *erro) {
        ...
        if ([result isKindOfClass:[NSArray class]]) {
            ...
            for (NSDictionary *aDic  in result_arr) {
                ServiceClass *aclass = [[ServiceClass alloc]initWithDictionary:aDic];
                
                [DataManager addService:aclass.pid serviceName:aclass.pname];
                
                for (NSDictionary *subDic in aclass.content) {
                    
                    ServiceClass *class_sub = [[ServiceClass alloc]initWithDictionary:subDic];
                    
                    [DataManager addServiceSubId:class_sub.id regionName:class_sub.name parentId:aclass.pid];
