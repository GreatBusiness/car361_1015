...
#import "DataBase.h"
#define kDBFileName @"CarDB.sqlite"
...
+(sqlite3 *)openDB
{
	...
	//原始sql文件路径
	NSString *originFilePath = [[NSBundle mainBundle] pathForResource:@"CarDB" ofType:@"sqlite"];
	
	NSFileManager *fm = [NSFileManager defaultManager];//文件管理器
	if([fm fileExistsAtPath:sqlFilePath] == NO)//如果sql文件不在doc下，copy过来
	{
		NSError *error = nil;
		if([fm fileExistsAtPath:originFilePath] && [fm copyItemAtPath:originFilePath toPath:sqlFilePath error:&error] == NO)
		{
